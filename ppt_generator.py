...
def generate_plan_with_claude(
    ...
    system = (
        ...
        "② subtitle(중제목): 같은 대제목(챕터) 안에서 슬라이드들을 묶는 세부 분류 이름.\n"
        "   - 해당 챕터에 슬라이드가 1개뿐이면: 비워둠 (empty string)\n"
        "   - 해당 챕터에 슬라이드가 여러 개이고 같은 묶음이면: 동일한 중제목 사용\n"
        "   - 해당 챕터 내 다른 묶음이면: 다른 중제목 사용\n"
        "   - 중제목은 숫자가 아닌 의미 있는 분류명으로 작성 (예: '시장 동향', '기술 구조', '도입 전략')\n"
        "   - 같은 중제목 아래 슬라이드가 여러 장이면 section_title로 구분\n"
